...
const Modify = () => {
  ...
  const pre_detailImg = new Array(3).fill('');
  var i = 0;
  for (var img of pre_data.detail_image.detail_image) {
    pre_detailImg[i] = img;
    i++;
  }
  ...
  return (
    <Container>
      ...
      <Content>
        ...
        <BottomContent>
          <div> * 이미지 변경 시 모든 이미지를 재등록해 주세요.</div>
          {detailImageCount.map((i, index) => {
            return (
              <Fragment key={index}>
                {detailPreview.length === 0 ? (
                  <DetailImageEx bgUrl={pre_detailImg[i].file} />
                ) : (
